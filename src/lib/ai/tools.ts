...
export const tutorTools: Anthropic.Messages.Tool[] = [
  ...
  },
  {
    ...
    description:
      "Generate an instant 10-question warmup quiz on the student's weakest topic. Call this ONLY for a GENERIC request with no topic named — 'quiz me', 'test me', 'take a mock now', 'practice me', 'give me questions to solve', 'I want to take a mock'. If the student names a specific topic or subject for the mock (e.g. 'geography mock test', 'test on number system and ratio', 'polity paper'), do NOT call this — give the pre-filled builder link instead, per the MOCK REQUESTS rule in the syllabus block. Creates ONE mock (a 10-question warmup) and saves it to their results history. (A full-length adaptive mock is NOT created here — if the student asks for one after the warmup, call the tool again or guide them to /exams/[code].) Do not call this if the student is mid-conversation about a specific concept and just wants explanation. CRITICAL: When this tool returns, output ONLY the `message_to_user` field verbatim as your entire response — do not add a preface, do not list extra options, do not invent claims about additional mocks. The message_to_user already contains the correct link and framing.",
    input_schema: {
      type: "object",
      properties: {
        topic_hint: {
