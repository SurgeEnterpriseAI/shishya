...
- **Public page by page (26 Sep 2026).** Board, class and subject pages list the official structure with its links and are indexable (a subject with no book, no chapter and no council document is not — `isSchoolSubjectIndexable` in `src/lib/school/surface.ts`). A chapter page is indexable, and in the sitemap, `llms-full.txt` and `context.md`, only when it holds Shishya's own notes or at least 5 answer-checked practice questions (`isSchoolChapterIndexable` in `src/lib/school/scope.ts`); a bare chapter page stays `noindex` and says its notes and practice are not ready yet. The page metadata and the sitemap read the same rule.
